...
function nav(view){$$(".view").forEach(x=>x.classList.remove("active"));$("#"+view).classList.add("active");$$("nav button").forEach(x=>x.classList.toggle("active",x.dataset.view===view))}
...
Regionale Priorität: ${region}.${focus?`\nZusätzlicher Fokus: ${focus}.`:""}
...
$("#copyPrompt").onclick=()=>copy("#promptOutput"); $("#copyMail").onclick=()=>copy("#mailOutput");
...
$("#saveCandidate").onclick=()=>{
 const name=$("#cName").value.trim(); if(!name)return alert("Bitte einen Namen eintragen.");
 data.candidates.push({id:crypto.randomUUID(),name,institution:$("#cInstitution").value.trim(),topic:$("#cTopic").value.trim(),period:$("#cPeriod").value.trim(),email:$("#cEmail").value.trim(),source:$("#cSource").value.trim(),why:$("#cWhy").value.trim(),question:$("#cQuestion").value.trim()});
 ["#cName","#cInstitution","#cTopic","#cPeriod","#cEmail","#cSource","#cWhy","#cQuestion"].forEach(x=>$(x).value="");
 $("#candidateForm").classList.add("hidden");persist();
};
...
Detlef Hau
TH Lübeck`;
 const a=data.acquisition[c.id]||{};$("#aStatus").value=a.status||"noch nicht angeschrieben";$("#aNote").value=a.note||"";
}
$("#aCandidate").onchange=makeMail;
$("#saveAcquisition").onclick=()=>{const c=selected("#aCandidate");if(!c)return alert("Bitte Kandidaten auswählen.");data.acquisition[c.id]={status:$("#aStatus").value,note:$("#aNote").value};persist()};
$("#iCandidate").onchange=()=>{const c=selected("#iCandidate");$("#iNotes").value=c?(data.interviews[c.id]||""):""};
$("#saveInterview").onclick=()=>{const c=selected("#iCandidate");if(!c)return alert("Bitte Kandidaten auswählen.");data.interviews[c.id]=$("#iNotes").value;persist()};
$("#zCandidate").onchange=()=>{const c=selected("#zCandidate");const z=c?(data.zDrafts[c.id]||{}):{};$("#zTitle").value=z.title||"";$("#zSummary").value=z.summary||"";$("#zSource").value=z.source||(c?c.source:"")};
$("#saveZ").onclick=()=>{const c=selected("#zCandidate");if(!c)return alert("Bitte Kandidaten auswählen.");data.zDrafts[c.id]={title:$("#zTitle").value,summary:$("#zSummary").value,source:$("#zSource").value};persist()};
$("#clearDemo").onclick=()=>{if(confirm("Alle lokal im Browser gespeicherten Demo-Daten löschen?")){storage.clear();data=storage.load();renderAll();nav("research")}};
